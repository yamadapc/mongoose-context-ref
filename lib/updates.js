...
exports.add = function add(schema, normalize) {
  ...
  schema.pre('save', function(next) {
    ...
    // whether the document has applicable contexts
    var original = this._original;
    var has_contexts = typeof this.context_id !== 'undefined' ||
                       (original && typeof original.context_id !== 'undefined');

    // whether the context has changed
    var context_is_modified = this.isModified('context_id') ||
                              this.isModified('context_type');

    if(is_new || !has_contexts || !context_is_modified) return next();
