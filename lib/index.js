...
module.exports = function(schema, options) {
  ...
  if(Array.isArray(options.context_types) && options.virtuals) {
    addVirtuals(schema, options.context_types, options.camel_case);
  }
  ...
  if(options.refUpdate) {
    schema.pre('save', function(next) {
      ...
      var path = getPath(this, options.camel_case);
      refUpdate('save', path, this, next);
    });
    ...
    schema.pre('remove', function(next) {
      ...
      var path = getPath(this, options.camel_case);
      refUpdate('remove', path, this, next);
    });
  }
...
function refUpdate(method, path, doc, cb) {
...
function addVirtuals(schema, context_types, camel_case) {
  context_types.forEach(function(context_type) {
    var path = camel_case ? serialization.uncapitalize(context_type):
                            serialization.snakelize(context_type);
...
function getPath(doc, camel_case) {
  var model_name = doc.constructor.modelName;
  return camel_case ? serialization.uncapitalize(model_name) + 's':
                      serialization.snakelize(model_name) + 's';
}
